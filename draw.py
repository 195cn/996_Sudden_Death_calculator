...
def draw_HP():
    ...
    for i in range(len(y)):
        if y[i] > 0:
            plt.text(x[i], y[i] + 1, ' %s' %round(y[i],3), ha='left', color = "green", fontsize=12)
        else :
            plt.text(x[i], y[i] + 1, ' %s' %round(y[i],3), ha='left', color = "black", fontsize=12)

    for i in range(len(yc1)):
        if yc1[i] != 0:
            plt.text(x[i], yc1[i]/2, '%s' %round(yc1[i],3), ha='center', color = "white", fontsize=8)

    for i in range(len(yc2)):
        if yc2[i] != 0:
            plt.text(x[i], yc2[i]/2, '%s' %round(yc2[i],3), ha='center', color = "white", fontsize=8)
